...
To determine the optimal structure for diabetic retinopathy screening coverage (DRSC) and diabetic glycemic control coverage (DGCC), models were analyzed across two time periods (2011-2023 and 2011-2019) while maintaining a fixed number of classes. A seven-class model was employed for both the full period (2011–2023) and the truncated baseline period (2011–2019) for DGCC, whereas a six-class model was used for DRSC. Models were selected based on the lowest Bayesian Information Criterion (BIC). Specifically, for DGCC, the seven-class cubic random effects model (2011-2023) exhibited the lowest BIC of -9137.468, while the seven-class quadratic random effects model (2011-2019) achieved a BIC of -6699.831. For DRSC, the seven-class cubic random effects model (2011-2023) yielded the lowest BIC of -2144.137, and the six-class linear homoscedastic model (2011-2019) had a BIC of -1635.550. These BIC values suggest that the cubic and quadratic models effectively captured complex, nonlinear trajectories for DGCC and DRSC during the full observation period (2011-2023), whereas the linear homoscedastic model indicated a simpler, more stable pattern for DRSC in the truncated period (2011-2019).

### **Model adequacy assessment**

Following the application of model adequacy criteria (Lowest APPA \> 0.70, OCC \> 5, entropy \> 0.6, minimum class size \> 2%, and Vuong-Lo-Mendell-Rubin Likelihood Ratio Test which compares models with k and k−1 classes, rejecting the latter if p \< 0.05), the final models for diabetic glycemic control coverage (DGCC) and diabetic retinopathy screening coverage (DRSC) differed across periods and specifications. To robust our results, we selected the two models with the lowest BIC values among those that met all adequacy thresholds, and compared them to determine which specification offered the most appropriate latent structure.

#### DGCC Models (2011–2023)

Two non-random effect cubic models with three and four latent classes, respectively, met all adequacy thresholds. The four-class model had the lowest BIC (−8,771.2), while the three-class model showed a slightly higher BIC (−8,663.7), with a ΔBIC of 107.5. The fourth class added in the more complex model remained above the minimum size threshold (4.7%) and retained strong adequacy (APPA = 0.898, OCC = 8.12, entropy = 0.836). The three-class solution had higher classification quality overall (APPA = 0.922, entropy = 0.847) and more balanced class proportions. Both models performed well, and while the four-class model showed better fit, the three-class model demonstrated stronger classification certainty and parsimony. Both were retained for subsequent comparative interpretation.

#### DGCC Models (2011–2019)

In this restricted period, the two-class cubic random effects model outperformed the alternative seven-class specification both in terms of fit and adequacy. The two-class model had the **lowest BIC** (−6,708.4), the highest entropy (0.887), and posterior probability (0.887), with both classes comfortably above the size threshold. The seven-class model showed a higher BIC (−6,494.8) and a lower minimum class size (2.08%), alongside a decline in entropy (0.801) and posterior probability (0.783). Based on these criteria, the two-class model was retained as the single favoured solution for this period.

#### DRSC Models (2011–2023)

For the DRSC dataset covering the full period, both the four-class cubic non-random effects model and the seven-class quadratic model met the adequacy requirements. The four-class model demonstrated a better fit (BIC = −2,076.2) and achieved higher classification performance (APPA = 0.854, entropy = 0.791), with no small classes. The seven-class model, while also meeting all adequacy thresholds, presented weaker classification certainty (APPA = 0.752, entropy = 0.759) and introduced several smaller classes, some just above the minimum size. Given these differences, both models were retained for analytical comparison, with the four-class model emerging as the stronger and more stable candidate.

#### DRSC Models (2011–2019)

Within the DRSC models estimated for the 2011–2019 period, two four-class quadratic random effects models were evaluated: one with unrestricted variance and one with proportional variance structure. The proportional model had a slightly lower BIC (−1,677.4 vs. −1,675.2), and also showed better classification adequacy in terms of entropy (0.667) and posterior probability (0.773). Both models had similar class distributions and met all adequacy conditions. While both were retained, the proportional variant was considered the more robust solution due to its improved classification performance and slightly better fit.
...
class_solutions <- class_solutions %>%
  ...
  mutate(across(
    c("Maximum log-likelihood", "AIC", "BIC", "SABIC", "entropy", "ICL1", "ICL2", 
      "X.class1", "X.class2", "X.class3", "X.class4", "X.class5", "X.class6", "X.class7",
      "Smallest class size (%)", "Smallest class count (n)", 
      "Lowest_OCC", "Lowest_APPA", "Highest_Mismatch"),
    ~ round(., 2)
  )) %>%
  # Keep only models matching your two patterns
  filter(str_detect(Model, "linear_nre_homocedastic_dgcc_model_2011_2023")|
         str_detect(Model, "linear_nre_homocedastic_dgcc_model_2011_2019")|
         str_detect(Model, "linear_nre_homocedastic_drsc_model_2011_2023")|
         str_detect(Model, "linear_nre_homocedastic_drsc_model_2011_2019")) %>%
  # Extract class number from the Model string
  mutate(class_number = as.numeric(str_extract(Model, "(?<=_)\\d(?=class)"))) %>%
  # Arrange first by model type, then by class number
  arrange(str_detect(Model, "2011_2019"), class_number) %>% 
gt::gt() %>%
#tab_spanner(label = 'Model fit and diagnostic criteria', columns = 2:16) %>%
tab_options(
table.font.size = 10,
