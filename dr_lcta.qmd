...
We assessed model discrimination using degrees of separation (DoSK) and Elsensohn’s envelope of residuals. To describe latent trajectory separation, we applied multivariate Mahalanobis distance to help identifying heterogeneous latent trajectories. Thus, higher DoSK values indicate well-separated mean trajectories, while a DoSK of zero indicates identical trajectories suggesting selection for alternative models with fewer classes.

We extended Elsenhohn et al.'s method used in fixed effects to random effects models. We calculated residuals for each participant and class-specific, time-specific weighted variances using posterior probabilities. We then plotted the upper and lower boundaries of the residuals' local SDs for each class. Non-parallel boundaries indicated heteroscedasticity, suggesting poor model fit, while varying interval widths pointed to unaccounted class variability. This method helped identify potential model issues, guiding necessary refinements for better fit.
...
The sensitivity analyses were carried out by replicating the modeling on subsets of the population with at least three time-point observations to test the stability of the classifications. After this step, we will assess the interpretability of the favored model.
...
data_summary3 <- data_summary3 %>% 
  ...
  mutate(across(4:8, ~ case_when(
    ...
    TRUE ~ round(. * 100, 1)             # Round other rows after multiplying by 100
  ))) %>%
...
# Print table using gt
data_summary6_gt <- data_summary6 %>% 
  dplyr::select(Metric, -1, 2:8) %>% 
  gt() %>%
  tab_options(
    table.font.size = 10,
