...
-   **DGC Coverage (DGCC):** The Proportion of T2DM patients achieving HbA1c \<7% among those diagnosed and monitored by the cardiovascular program.
...
-   **Deprivation:** The Chilean Observatory of Public Health developed the Index of Socioeconomic Development (ISED) score, which measures deprivation from 0 to 1, with lower scores indicating greater deprivation. This index encompasses income, education, housing, and sanitation.[@gattini_comunas_2013] To meet the distributional assumptions necessary for subsequent analyses, we standardized the ISED, resulting in a modified scale with an expanded range of values.
...
-   In the first stage, we employed a Latent Class Mixed Model (LCMM) to identify trajectory classes for DRSC and DGCC from 2011 to 2023. LCMM probabilistically assigns latent classes representing subgroups or unique patterns within a population.[@berlin_introduction_2014]

    Following a eight step framework, we built a latent class trajectory model, using the step description and criteria for selection[@lennon2018] (See supplementary Material):

    -   1\. We initially fitted a scope models to determine the random effects structure of the modelling by examining the shape of the standarsized residuals in a simple linear model with no random effects following the rationale of Verbeke and Molenbergh.[@molenberghs_model_2009] A scoping model might be built based on the plausible number of classes found in previous literature. As we had no available literature about this kind of trajectories studies, we did not hypothesized regarding the class number. We examined the plotted shape of the standarised residuals from these derived models for each of the classes from 1 to 7 seven class-models. Residual patterns guide the selection of a suitable random effects structure. A horizontal line suggests a random intercept model suffices. A diagonal line indicates a linear random effect may be appropriate. A curved pattern suggest a quadratic random effect, while an s-shaped pattern supports a cubic random effect. These visual cues determine the necessary complexity for the random effects component of the model.

    -   2\. We use two mail tools: BIC and sample size. We explored model complexity from one to seven classes,[@lennon2018] in our preliminary working models. As we had no clue about of the plausible number, we use the BIC as the main tool for selecting the optimal class number. The nature of this research where the number of individuals (municipalities) is limited, so in studies with larger number suggested 1% as plausible. However we have assumed at this stage that models representing a sample size under 3% were (less than nine municipaties) were discarded before hand.

    -   3\. Once we defined our preliminary working model and the plausible optimal number of classes, we refined the model structure in the selected class memebership of our working model in eight different modelling assumptions checking; 1) fixed linear effects homocedasctic; 2) fixed linear effects heterocedastic; 3) Random intercept; 4) Random intercept slope; 5) Random quadratic, equal; 6) Random quadratic proportional; and 7) Random cubic, equal, and; 8)Random cubic, proportional. (See table X).

    -   4\. Once selected the model structure best fits, we assesed a complete mpodel adequacy assesment. Foe each municipality we calculated the posterior probability to be assigned to each trajectoryminimun Average Latent Class Posterior Probability (ALCPP) including Akaike Information Criteria (AIC), sample-adjusted Bayesian Information Criteria (SABIC), Consistent Akaike Information Criterion (CAIC), Approximate Weight of Evidence Criterion (AWE), Bayes Factor (BF), and correct model probability (cmP). Diagnostic tests included entropy, smallest class count (SCC), size (SCS), and , along with the Vuong-Lo-Mendell-Rubin (VLMLRT) test to compare the models; we favored the k-class model over the k-1 class model when the p-value was below 0.05, considering clinical plausibility, model fit, and diagnostic criteria.[@wickrama2021]

    -   5\. We plotted mean trajectories for each class in a single graph and then plotted individually every class with their respective confidence interval (CI), as well as a individual class spaghetti plots for a random sample.

    -   6\. We run additional tools to further assess discrimintaion including Degrees of Separation (DoS) and Elsensohn’s envelope of residuals. INCLUIR LOS MODELOS DE GRAFICOS DE STEP 1 EN STEP 6

    -   7\. We consider epidemiological plausibility characterisation. Ideally classes not be too skewed with some sample sizes less than arguing about less than 5% of the sample, but our sample is still keep a significant amount of municipalities under 8. We conducted a sensitivity analysis by default covering 2011 to 2019 to assess the potential impact of excluding the pandemic years (2019–2023) on the DRSC and DGCC trajectories. We also assessed data completeness across observations over time, comparing means using Welch’s t-test to ensure balanced data distributions across trajectory classes.

    In addition reported our LCMM analysis using the guidelines for reporting on latent trajectory studies (GRoLTS).[@vandeschoot2016]
...
All statistical analyses were performed using R version 4.3.1 and the “lcmm” package version 2.1.0.
...
Figure 2 presents the LCMM results for DRSC, where two classes are identified:
