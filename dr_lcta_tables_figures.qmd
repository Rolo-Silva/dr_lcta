...
To identify the number of latent trajectories, we first constructed exploratory scoping models that suggested a four-class solution for both outcomes across periods. BIC curves from linear fixed-effects models with one to seven classes (Structure A) consistently showed a distinct elbow at four classes, indicating that further class addition yielded diminishing improvements in model fit. In parallel, standardized residual plots from the one-class linear models suggested parabolic and cubic patterns, pointing to unaccounted nonlinear trends and the likely need for higher-order structures (see Supplementary Figure S1). Although these patterns emerged independently, both provided complementary justification for adopting a four-class structure as the analytical basis for subsequent model refinement.

**Model structure refinement**

**We refined the model structure by testing ten predefined variants to improve fit and interpretability while retaining the four-class solution.** Each structure (A–J) varied in functional form, random effects, and error variance assumptions. We assessed them using BIC, classification quality, and theoretical coherence. Structures that failed to meet adequacy thresholds or produced uninterpretable classes were discarded (see Supplementary Table S2 and Figures S1–S4). This process confirmed the four-class configuration as robust across multiple specifications, setting the foundation for assessing consistency across outcomes and periods.

To determine the optimal structure for each outcome and timeframe, we applied all ten models (A–J) to latent class solutions ranging from one to seven classes described in Supplementary Table S1 (labeled A–J). Specifically, for DGCC, the seven-class cubic random effects model (2011–2023) had the lowest BIC (–9137.5), while the seven-class quadratic random effects model (2011–2019) reported a BIC of –6699.8. For DRSC, the seven-class cubic random effects model (2011–2023) yielded a BIC of –2144.1, whereas the six-class linear homoscedastic model (2011–2019) achieved –1635.6. These results suggested that quadratic and cubic models captured complex nonlinear trajectories for DGCC and DRSC in the full observation period (2011–2023), while linear models reflected simpler, more stable patterns in the shorter timeframe (2011–2019).

**Model adequacy assessment**

Across all four retained models—covering both outcomes and periods—a consistent four-class structure was identified as optimal. This convergence underscores shared adequacy in classification quality, relative entropy, and interpretability, even as model structures varied: while non-random effects structures were favored for both periods in DGCC, a proportional random effects structure was required to achieve adequacy for DRSC in 2011–2019. In all cases, BIC elbow plots (Supplementary Figures S3–S6) exhibited a clear inflection between three- and four-class solutions, further validating the choice. DGCC models displayed higher relative entropy (0.83–0.85) and OCC scores (8.1–11.7), alongside relatively balanced class sizes. DRSC models showed greater dispersion in entropy and separation metrics, with the 2011–2019 model having the lowest entropy (0.67) and OCC (5.5), though still above the minimum thresholds. Importantly, several lower-BIC models were discarded for failing adequacy criteria, including class imbalance and weak classification. These selections confirm that a four-class configuration provides a robust, interpretable framework for capturing latent coverage trajectories across both glycemic control and retinopathy screening outcomes. The consistent elbow inflection at four classes reinforces the theoretical plausibility and empirical stability of this structure across periods and model families.
...
model_adequacy_table %>%
  ...
  tab_header(
    title = md("**Table 1. Description of the 10 candidate model structures tested in the LCMM analysis**"))
