...
```{r}
#| label: tbl-S1
#| tbl-cap: "Different models structures with increasing complexity"
#| fig-align: left
# Table S1 - Model types, interpretation, software and references

model_comparison_table <- data.frame(
  Model = c("A", "B", "C", "D", "E", "F", "G"),
  Description = c(
    "Fixed effects homoscedastic (common residual variance across classes)",
    "Fixed effects heteroscedastic (class-specific residual variances)",
    "Random intercept",
    "Random slope",
    "Random quadratic – Common variance structure across classes",
    "Random quadratic – Proportionality constraint (variance structures vary across classes)",
    "Random quadratic – Class-specific variance structure (unstructured)"
  ),
  Interpretation = c(
    "No random effects – any deviation of an individual’s trajectory from its mean class trajectory is due to random error only.",
    "Same as Model A, but random errors can differ in size across classes.",
    "Allows individuals to vary in initial weight; each class member follows the same shape and magnitude of the mean trajectory.",
    "Allows individuals to vary in initial weight and slope, but same curvature as the trajectory.",
    "Allows individuals to vary in initial weight, shape, and magnitude. Variability is common across all classes.",
    "Allows variance structures to differ across classes up to a multiplicative factor (parsimonious version of Model G).",
    "Each class has its own separate random quadratic variance structure; full flexibility in variance and covariance of intercept, slope, and quadratic term."
  )
)
...
# Define model structure metadata
structure_table <- data.frame(
  Label = LETTERS[1:10],
  Structure = c(
    "linear nre homocedastic",
    "linear nre heterocedastic",
    "quadratic nre",
    "cubic nre",
    "linear random intercept",
    "linear random intercept slope",
    "quadratic random effects",
    "quadratic random effects prop",
    "cubic random effects",
    "cubic random effects prop"
  ),
  Fixed_effects = c(
    "1 + year",
    "1 + year",
    "1 + year + I(year^2)",
    "1 + year + I(year^2) + I(year^3)",
    "1 + year",
    "1 + year",
    "1 + year + I(year^2)",
    "1 + year + I(year^2)",
