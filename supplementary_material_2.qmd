...
#| paged-print: false
#| fig-align: left
#| fig-cap: "Figure S5. Observed DGCC and DRSC trajectories and Elsensohn envelopes residuals plots" 
...
dgcc_title <- patchwork::wrap_elements(grid::textGrob(
  "Observed DGCC residuals Elsensohn envelopes plots", 
  x = 0, hjust = 0, gp = grid::gpar(fontface = "bold", fontsize = 8)
))

drsc_title <- patchwork::wrap_elements(grid::textGrob(
  "Observed DRSC residuals Elsensohn envelopes plots", 
  x = 0, hjust = 0, gp = grid::gpar(fontface = "bold", fontsize = 8)
))


dgcc_residual_plot <- dgcc_title / residual_plot_list[[1]]  + plot_layout(heights = c(0.1, 1))
drsc_residual_plot <- drsc_title / residual_plot_list[[2]]  + plot_layout(heights = c(0.1, 1))
  

# Final combination
elsensohn_plots <- dgcc_residual_plot | drsc_residual_plot

# Optional overall annotation
elsensohn_plots <- elsensohn_plots +
  plot_annotation(
    theme = theme(
...
# Read results
sens_dgcc_original <- readRDS("sens_dgcc_original_seed123.rds")
sens_dgcc_90 <- readRDS("sens_dgcc_prop90_seed123.rds")
sens_dgcc_80 <- readRDS("sens_dgcc_prop80_seed123.rds")
sens_dgcc_70 <- readRDS("sens_dgcc_prop70_seed123.rds")

sens_drsc_original <- readRDS("sens_drsc_original_seed123.rds")
sens_drsc_90 <- readRDS("sens_drsc_prop90_seed123.rds")
sens_drsc_80 <- readRDS("sens_drsc_prop80_seed123.rds")
sens_drsc_70 <- readRDS("sens_drsc_prop70_seed123.rds")

# Helper to set a concise title on either a list-with-$plot or a ggplot
simple_plot <- function(x, title) {
  p <- if (inherits(x, "gg")) x else x$plot
  p + labs(title = title) +
    theme(plot.title = element_text(face = "bold", size = 10))
}

# DGCC panel: Original | 90% | 80% | 70%
panel_dgcc <- (
  simple_plot(sens_dgcc_original, "Original")    |
  simple_plot(sens_dgcc_90,       "90% subsample") |
  simple_plot(sens_dgcc_80,       "80% subsample") |
  simple_plot(sens_dgcc_70,       "70% subsample")
) +
  plot_layout(guides = "collect") +
  plot_annotation(title = "DGCC — Original, 90%, 80%, 70% subsamples",
...
# DRSC panel: Original | 90% | 80% | 70%
panel_drsc <- (
  simple_plot(sens_drsc_original, "Original")    |
  simple_plot(sens_drsc_90,       "90% subsample") |
  simple_plot(sens_drsc_80,       "80% subsample") |
  simple_plot(sens_drsc_70,       "70% subsample")
) +
  plot_layout(guides = "collect") +
  plot_annotation(title = "DRSC — Original, 90%, 80%, 70% subsamples",
                  theme = theme(plot.title = element_text(size = 12, face = "bold"),
                                legend.position = "bottom",
                                legend.direction = "horizontal",
...
# 1) Load data -----------------------------------------------------------------
coverage_data    <- read_csv("coverage_data.csv", show_col_types = FALSE)
all_named_models <- readRDS("all_named_models.rds")

demographics <- read_csv("demographics.csv", show_col_types = FALSE) %>%
  mutate(
    deprivation_index = index_standardized,
...
# 2) Limit to the two models you care about ------------------------------------
target_models <- c("4class_cubic_nre_dgcc_model", "4class_cubic_nre_drsc_model")
missing <- setdiff(target_models, names(all_named_models))
if (length(missing)) stop("Missing models in all_named_models: ", paste(missing, collapse = ", "))

# Make sure each stored model points to the actual coverage_data (for safety)
for (nm in target_models) all_named_models[[nm]]$call$data <- coverage_data

# 3) Build merged data per model, with stable class ordering -------------------
build_merged <- function(model_name) {
  model <- all_named_models[[model_name]]
  stopifnot(!is.null(model$call$data), !is.null(model$pred), !is.null(model$pprob))

  # we only need id & outcome to compute class ordering
  outcome_var <- if (grepl("drsc", model_name, ignore.case = TRUE)) "drsc" else "dgcc"

  model_data <- model$call$data %>%
    arrange(id, year) %>%
    group_by(id) %>% mutate(row_id = row_number()) %>% ungroup()
  ...
  # order classes by mean outcome → 1..K small→large (stable across plots/tables)
  class_order <- merged_df %>%
    group_by(class) %>%
    summarise(mean_value = mean(.data[[outcome_var]], na.rm = TRUE), .groups = "drop") %>%
    arrange(mean_value) %>%
    mutate(class_order = row_number())
...
all_merged_df <- lapply(setNames(target_models, target_models), build_merged)

# 4) Correct id → comuna map (CRITICAL fix) ------------------------------------
id_to_comuna <- coverage_data %>% distinct(id, comuna)
# sanity checks
stopifnot(nrow(id_to_comuna) == n_distinct(coverage_data$id))
if (nrow(anti_join(id_to_comuna, demographics, by = "comuna")) > 0) {
  warning("Some comunas in coverage_data are missing in demographics; those rows will be dropped.")
}

# 5) Run multinomial regressions for each model --------------------------------
regression_results <- list()
...
for (model_name in names(all_merged_df)) {
  df <- all_merged_df[[model_name]] %>%
    ...
    left_join(id_to_comuna, by = "id") %>%
    left_join(demographics, by = "comuna") %>%
    drop_na(deprivation_index, urbanisation_classification, zona)

  if (n_distinct(df$class) < 2) next

  df$class <- factor(df$class)
  # use highest numeric class as reference
  df$class <- stats::relevel(df$class, ref = as.character(max(as.numeric(as.character(df$class)))))

  model_fit <- suppressWarnings(suppressMessages(
    nnet::multinom(class ~ deprivation_index + urbanisation_classification + zona, data = df)
  ))
...
# --- replace your create_clean_table() with this -----------------------
create_clean_table <- function(df, model_name) {
  labels <- trajectory_labels[[model_name]]
  if (is.null(labels)) stop("No labels for ", model_name)
  ...
  out <- df %>%
    ...
    dplyr::select(term, trajectory, RRR_CI) %>%
    tidyr::pivot_wider(names_from = trajectory, values_from = RRR_CI) %>%
    clean_term_labels()

  # add the (reference) last class column if it isn't present
  ref_col <- labels[length(labels)]
  if (!ref_col %in% names(out)) out[[ref_col]] <- "Ref"

  # order the class columns exactly as in `labels`
  out %>% dplyr::select(term, dplyr::all_of(labels))
}

# 7) Build the two final tables -------------------------------------------------
tab_dgcc <- create_clean_table(final_regression_table, "4class_cubic_nre_dgcc_model") %>%
  add_reference_rows()

tab_drsc <- create_clean_table(final_regression_table, "4class_cubic_nre_drsc_model") %>%
  add_reference_rows()
...
gt_dgcc <- tab_dgcc %>%
  ...
  tab_header(
    title = md("**Table S5: Odds ratios and 95% confidence intervals for DGCC 2011–2023**"),
    subtitle = md("Multinomial logistic regression: membership in each class vs **Highest** (reference)")
  )
...
gt_drsc <- tab_drsc %>%
  ...
  tab_header(
    title = md("**Table S6: Odds ratios and 95% confidence intervals for DRSC 2011–2023**"),
    subtitle = md("Multinomial logistic regression: membership in each class vs **Highest decreasing** (reference)")
  )

gt_dgcc
gt_drsc

# --- Optional: DRSC using "Increasing" as the reference -----------------------
# (uncomment to produce an alternative table)
# {
#   model_name <- "4class_cubic_nre_drsc_model"
#   labels <- trajectory_labels[[model_name]]
#   ref_label <- "Increasing"
#   ref_index <- match(ref_label, labels)
#
#   df_alt <- all_merged_df[[model_name]] %>%
#     distinct(id, class) %>%
#     left_join(id_to_comuna, by = "id") %>%
#     left_join(demographics, by = "comuna") %>%
#     drop_na(deprivation_index, urbanisation_classification, zona) %>%
#     mutate(class = factor(class, levels = 1:length(labels), labels = labels),
#            class = relevel(class, ref = ref_label))
#
#   fit_alt <- suppressWarnings(suppressMessages(
#     nnet::multinom(class ~ deprivation_index + urbanisation_classification + zona, data = df_alt)
#   ))
#
#   tab_drsc_increasing <- broom::tidy(fit_alt, exponentiate = TRUE, conf.int = TRUE) %>%
#     mutate(
#       RRR_CI = paste0(round(estimate, 2), " [", round(conf.low, 2), ", ", round(conf.high, 2), "]"),
#       y.level = factor(y.level, levels = labels[-ref_index])
#     ) %>%
#     select(term, y.level, RRR_CI) %>%
#     pivot_wider(names_from = y.level, values_from = RRR_CI) %>%
#     mutate(`{ref_label}` := "Ref") %>%
#     select(term, all_of(labels)) %>%
#     clean_term_labels() %>%
#     add_reference_rows() %>%
#     gt() %>%
#     tab_header(
#       title = md("**Table S6 (alt): DRSC with 'Increasing' as reference**")
#     )
#
#   tab_drsc_increasing
# }
...
demographics <- read_csv("demographics.csv", show_col_types = FALSE) %>%
  mutate(
    deprivation_index = index_standardized,
    # set the exact target references
    urbanisation_classification = factor(urbanisation_classification,
                                         levels = c("Urbana","Mixta","Rural")),
    zona = factor(zona, levels = c("centro","norte","sur"))
  ) %>%
...
# Make sure every model uses your coverage_data (some saved models keep old env)
for (mn in names(all_named_models)) {
  if (!is.null(all_named_models[[mn]]$call))
    all_named_models[[mn]]$call$data <- coverage_data
}

# A reliable id -> comuna map taken from the *actual* modelling data
id_to_comuna <- coverage_data %>%
  distinct(id, comuna) %>%
  filter(!is.na(id), !is.na(comuna))

# ==== 2) PREP: build merged_df for the two target models ======================
target_models <- c("4class_cubic_nre_dgcc_model","4class_cubic_nre_drsc_model")

build_merged <- function(model_name) {
  model <- all_named_models[[model_name]]
  if (is.null(model) || is.null(model$call$data) || is.null(model$pred) || is.null(model$pprob)) {
    warning(paste("Missing components in", model_name))
    return(NULL)
  }
  ...
  outcome_var <- ifelse(grepl("drsc", model_name, ignore.case = TRUE), "drsc", "dgcc")
  # or: outcome_var <- ifelse(stringr::str_detect(model_name, stringr::regex("drsc", ignore_case = TRUE)), "drsc", "dgcc")

  class_order <- merged_df %>%
    group_by(class) %>%
    summarise(mean_val = mean(.data[[outcome_var]], na.rm = TRUE), .groups = "drop") %>%
    arrange(mean_val) %>%
    mutate(class_order = row_number())
...
all_merged_df <- lapply(target_models, build_merged)
names(all_merged_df) <- target_models
all_merged_df <- Filter(Negate(is.null), all_merged_df)

if (!all(target_models %in% names(all_merged_df))) {
  warning("One or both target models could not produce merged data; tables may be partial.")
}

# ==== 3) MULTINOMIAL RRR FITTER ==============================================
fit_multinom_rrr <- function(model_name, merged_df) {

  # collapse to one row per id: (id, class)
  df <- merged_df %>%
    distinct(id, class) %>%
    left_join(id_to_comuna, by = "id") %>%
    left_join(demographics, by = "comuna") %>%
    # make modelling inputs safe & drop NAs
    mutate(
      class = factor(class),  # 1..K after our ordering above
      # set the highest class as reference level:
      class = forcats::fct_relevel(class, as.character(max(as.numeric(as.character(class)), na.rm = TRUE)))
    ) %>%
    filter(!is.na(deprivation_index),
           !is.na(urbanisation_classification),
           !is.na(zona),
           !is.na(class)) %>%
    droplevels()

  if (nlevels(df$class) < 2) {
    warning(paste("Not enough outcome levels for", model_name, "— skipping"))
    return(NULL)
  }

  # ensure references for covariates
  df <- df %>%
    mutate(
      urbanisation_classification = fct_relevel(urbanisation_classification, "Urbana","Mixta","Rural"),
      zona  = fct_relevel(zona, "centro","norte","sur")
    )

  # Fit
  fit <- try(nnet::multinom(class ~ deprivation_index + urbanisation_classification + zona,
                            data = df, trace = FALSE), silent = TRUE)
  if (inherits(fit, "try-error")) {
    warning(paste("multinom() failed for", model_name))
    return(NULL)
  }
...
regression_results <- list()
for (mn in names(all_merged_df)) {
  regression_results[[mn]] <- fit_multinom_rrr(mn, all_merged_df[[mn]])
}
regression_results <- Filter(Negate(is.null), regression_results)

if (length(regression_results) == 0) {
  stop("No multinomial models could be fitted for the target models.")
}

final_regression_table <- bind_rows(regression_results)

# ==== 4) NICE LABELS + WIDE TABLE PER MODEL ==================================
trajectory_labels <- list(
  "4class_cubic_nre_dgcc_model" = c("Consistently lower","Stable medium","Stable upper medium","Highest"),
  "4class_cubic_nre_drsc_model" = c("Consistently low","Stable medium","Increasing","Highest decreasing")
)
...
# build a wide table (columns = classes vs reference, which is the highest)
make_table_for_model <- function(df_all, model_name) {
  labs <- trajectory_labels[[model_name]]
  if (is.null(labs)) stop("No trajectory labels for ", model_name)

  dfm <- df_all %>%
    filter(model_name == !!model_name) %>%
    mutate(trajectory = factor(y.level, levels = as.character(seq_along(labs)), labels = labs)) %>%
    select(term, trajectory, RRR_CI) %>%
    tidyr::pivot_wider(names_from = trajectory, values_from = RRR_CI)

  # Add explicit "Ref" in the (highest) reference column if missing
  ref_col <- labs[length(labs)]
  if (!ref_col %in% names(dfm)) dfm[[ref_col]] <- NA_character_
  dfm[[ref_col]][is.na(dfm[[ref_col]])] <- "Ref"

  dfm <- clean_term_labels(dfm)

  # add explicit reference rows for Urban/Centre
  ref_rows <- tibble(term = c("Urban (Ref)", "Centre (Ref)"))
  for (cc in setdiff(names(dfm), "term")) ref_rows[[cc]] <- "Ref"
  dfm <- bind_rows(dfm, ref_rows)

  # order rows as requested
  row_order <- c(
    "(Intercept)",
    "Deprivation Index (continuous)",
    "Rural (Ref: Urban)", "Mixta (Ref: Urban)", "Urban (Ref)",
    "Norte (Ref: Centre)", "Sur  (Ref: Centre)", "Centre (Ref)"
  )
...
table_dgcc <- make_table_for_model(final_regression_table, "4class_cubic_nre_dgcc_model")
table_drsc <- make_table_for_model(final_regression_table, "4class_cubic_nre_drsc_model")
...
gt_dgcc <- table_dgcc %>%
  ...
  tab_header(
    title = md("**Table S5. Multinomial RRR (95% CI) — DGCC (Ref: Highest class)**")
  )
...
gt_drsc <- table_drsc %>%
  ...
  tab_header(
    title = md("**Table S6. Multinomial RRR (95% CI) — DRSC (Ref: Highest class)**")
  )
